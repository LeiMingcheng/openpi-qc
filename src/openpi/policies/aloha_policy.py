...
class AlohaInputs(transforms.DataTransformFn):
    ...
    def __call__(self, data: dict) -> dict:
        ...
        # 保留RL训练必需的字段
        rl_fields = ["reward", "terminal", "next_state", "next_image", "episode_id"]
        for field in rl_fields:
            if field in data:
                inputs[field] = data[field]

        # 🆕 保留Q-chunking必需的pad字段（episode边界检测）
        pad_fields = ["actions_is_pad", "state_mask"]
...
def _decode_aloha(data: dict, *, adapt_to_pi: bool = False) -> dict:
    ...
    def convert_image(img):
        ...
        # 🔧 RL修复：处理时间序列图像数据，智能判断CHW格式
        if len(img.shape) == 4:
            # 4D: 检查是(T,C,H,W)还是(T,H,W,C)
            if img.shape[1] <= 4 and img.shape[1] < img.shape[2] and img.shape[1] < img.shape[3]:
                # (T, C, H, W) -> (T, H, W, C)
            ...
            raise ValueError(f"Unexpected image shape: {img.shape}, expected 3D or 4D")
