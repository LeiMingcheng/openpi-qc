...
_CONFIGS = [
    ...
    TrainConfig(
     ...
     # pi0_base by full
    TrainConfig(
        name="pi0_dual_clothes_full",
        model=pi0.Pi0Config(),
        data=LeRobotAlohaDataConfig(
            repo_id="yzysmile/aloha_fold_box",
            #repo_id="fold_box_unified",# your datasets repo_id
                                                   # repo_id决定计算哪个数据集的归一化统计量
            adapt_to_pi = False,
            ...
            base_config=DataConfig(
                ...
                prompt_from_task=False,  # 不根据lerobot数据集名称 给 语言指令
            ),
        ),
        batch_size=256, # 所有GPU上的样本数总和，而不是单个GPU上的批量大小
        weight_loader=weight_loaders.CheckpointWeightLoader("/dev/shm/lmc/openpi/checkpoints/pi0_base/openpi-assets/checkpoints/pi0_base/pi0_base/params"),
        num_train_steps=30000,
        fsdp_devices=8,  # 如果只有1张GPU，fsdp_devices=1
    ),
        # pi0_base by full
    TrainConfig(
        name="pi0_dual_box_full",
        model=pi0.Pi0Config(action_horizon=20),  # 当数据以15hz采集时，设置action_horizon=20
        data=LeRobotAlohaDataConfig(
            repo_id="fold_box_unified_score5",# your datasets repo_id (2324_total)
                                                   # repo_id决定计算哪个数据集的归一化统计量
            adapt_to_pi = False,
            ...
            base_config=DataConfig(
                prompt_from_task=False,  # 不根据lerobot数据集名称 给 语言指令 默认为false
            ),
        ),
        batch_size=32, # 所有GPU上的样本数总和，而不是单个GPU上的批量大小
        #weight_loader=weight_loaders.CheckpointWeightLoader("/dev/shm/lmc/openpi/checkpoints/pi0_base/openpi-assets/checkpoints/pi0_base/pi0_base/params/"),
        weight_loader=weight_loaders.CheckpointWeightLoader("/era-ai/lm/weight/pi0/pi0_dual_box_full/yzy_fold_box/90000/params/"),
        # /root/project/openpi/checkpoints/pi0_dual_clothes_full/2324_total/60000/params
