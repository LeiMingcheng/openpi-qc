...
def check_train_state_consistency(state1: Any, state2: Any,
                                 name1: str = "state1",
                                 name2: str = "state2") -> bool:
    """
    Check if two training states have consistent pytree structures.
    
    Args:
        state1: First training state
        state2: Second training state
        name1: Name for first state (for logging)
        name2: Name for second state (for logging)
    ...
    try:
        # Get treedef structures
        _, treedef1 = jax.tree_util.tree_flatten(state1)
        _, treedef2 = jax.tree_util.tree_flatten(state2)
        
        # Check if treedefs are equal
        if treedef1 == treedef2:
            logger.info(f"✅ Training state consistency check passed: {name1} ≡ {name2}")
            return True
        else:
            logger.error(f"❌ Training state inconsistency detected: {name1} ≠ {name2}")
            logger.error(f"  {name1} treedef: {treedef1}")
            logger.error(f"  {name2} treedef: {treedef2}")
            return False
            
    except Exception as e:
        logger.error(f"❌ Failed to check training state consistency: {e}")
        return False
...
if __name__ == "__main__":
    ...
    # Create two optimizers the same way
    opt1 = _optimizer.create_optimizer(rl_config.actor_optimizer, rl_config.actor_lr_schedule)
    opt2 = _optimizer.create_optimizer(rl_config.actor_optimizer, rl_config.actor_lr_schedule)
