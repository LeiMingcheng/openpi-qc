...
class ACRLPDHyperparams:
    ...
    # save_frequency已移除，统一使用RLTrainConfig.save_interval
    ...
    enable_batch_ratio_control: bool = False  # 启用batch级别比例控制
...
RL_FOLD_BOX = RLTrainConfig(
                ...
                _transforms.RepackTransform(
                    {
                        ...
                        "actions": "action",  # 统一使用LeRobot标准字段名
                        "reward": "reward",  # 添加reward字段映射，确保RL数据完整性
                        # 🆕 完整的pad字段映射（Q-chunking episode边界检测必需）
                        "actions_is_pad": "action_is_pad",
                        "image_mask": {
                            "cam_high": "observation.images.cam_high_is_pad",
                            "cam_left_wrist": "observation.images.cam_left_wrist_is_pad", 
                            "cam_right_wrist": "observation.images.cam_right_wrist_is_pad",
                        },
                        "state_mask": "observation.state_is_pad",
                    }
                )
            ]
        ),
    ...
    num_train_steps=200000,  # 与用户配置一致
    log_interval=20,
    save_interval=5000,
    ...
    acrlpd=ACRLPDHyperparams(
        ...
        # Behavior Cloning正则化
        bc_loss_weight=10,  # 适中的BC权重，适合manipulation任务
        ...
        # eval_frequency已移除 - 当前无真实环境评估实现
        # save_frequency已移除，使用RLTrainConfig.save_interval=5000
        ...
        ## 当前Epoch机制已经废弃（现在只依赖num_train_steps），学习率调节采用openpi标准的CosineDecaySchedule
