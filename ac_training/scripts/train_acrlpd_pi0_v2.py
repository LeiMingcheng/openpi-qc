#!/usr/bin/env python3
"""
Training script for ACRLPD + π₀ integration with OpenPI-based DataLoader.

Ultra-high-efficiency implementation:
- Direct OpenPI integration with zero-copy data transforms
- Vectorized Q-chunking format generation in _collate_fn  
- Simplified FSDP sharding with intelligent scalar field detection
- Fast-fail data sampling (no complex retry mechanisms)

Usage:
    python train_acrlpd_pi0_v2.py --config rl_fold_box --exp_name experiment_name
    python train_acrlpd_pi0_v2.py --config rl_fold_box --exp_name exp --resume
    python train_acrlpd_pi0_v2.py --config rl_fold_box --exp_name exp --no_wandb
"""
...
def setup_jax_environment_with_fsdp(args: argparse.Namespace, rl_config: RLTrainConfig):
    ...
    # Log initial GPU memory - 注释掉以加速初始化
    # log_gpu_memory("INITIAL")
    logger.info("✅ 跳过初始GPU内存监控以加速初始化")
    ...
    # Log GPU memory after FSDP setup - 注释掉以加速初始化
    # log_gpu_memory("FSDP_SETUP")
    logger.info("✅ 跳过FSDP setup后GPU内存监控以加速初始化")
...
def create_acrlpd_data_loader_v2_with_sharding(rl_config: RLTrainConfig, batch_size: int, data_sharding: jax.sharding.Sharding, replicated_sharding: jax.sharding.Sharding, debug_transforms: bool = False):
    """Create high-efficiency V2 ACRLPD data loader with FSDP sharding support."""
    # 使用V2高效dataloader，直接使用RL config的data配置
    
    # 直接使用V2 dataloader，它期望rl_config作为第一个参数
    ...
    logger.info(f"✅ Created V2 dataloader with OpenPI efficiency and Q-chunking support")
    logger.info(f"  Dataset: {rl_config.data.repo_id}")
    logger.info(f"  Batch size: {batch_size}")
    logger.info(f"  Action horizon: {rl_config.model.action_horizon}")
    logger.info(f"  Positive sample ratio: {getattr(rl_config.acrlpd, 'positive_sample_ratio', 0.1)}")
    ...
    # 包装V2数据加载器以支持FSDP分片
    class ShardedACRLPDDataLoaderV2:
        ...
        def __iter__(self):
            # V2 dataloader不支持迭代器，使用sample_batch方法
            raise NotImplementedError("V2 dataloader使用sample_batch()方法而非迭代器")
        
        def __len__(self):
            return len(self.base_loader)
            
        def sample_batch(self):
            """超高效OpenPI数据采样 + 标准FSDP分片"""
            
            try:
                batch = self.base_loader.sample_batch()
            except Exception as e:
                # 有任何问题直接重新采样，现在速度很快
                logger.debug(f"数据采样异常，直接重新采集: {type(e).__name__}")
                batch = self.base_loader.sample_batch()
            
            # 简化的FSDP分片：标量字段使用replicated，其他使用data分片
            def apply_sharding(path, x):
                path_str = '.'.join(str(p) for p in path) if isinstance(path, tuple) else str(path)
                x_array = np.asarray(x)
                
                # 标量字段：rewards, masks, terminals
                is_scalar = any(name in path_str for name in ['rewards', 'masks', 'terminals'])
                sharding = self.replicated_sharding if is_scalar else self.sharding
                return jax.make_array_from_process_local_data(sharding, x_array)
            
            result = jax.tree_util.tree_map_with_path(apply_sharding, batch)
            return result
...
def main():
    ...
    logger.info("🚀 Starting ACRLPD + π₀ training with V2 dataloader and OpenPI FSDP")
    ...
    try:
        ...
        # Create V2 data loader with FSDP sharding support
        logger.info("Creating V2 data loader with FSDP sharding...")
        dataloader = create_acrlpd_data_loader_v2_with_sharding(
            rl_config=rl_config,
            batch_size=rl_config.batch_size,
            data_sharding=data_sharding,
            replicated_sharding=replicated_sharding,
            debug_transforms=getattr(args, 'debug_transforms', False)
        )
        logger.info(f"V2 Data loader created with OpenPI efficiency and FSDP sharding support")
        ...
        logger.info(f"✅ 使用OpenPI标准学习率控制 (Actor: {rl_config.acrlpd.actor_lr}, Critic: {rl_config.acrlpd.critic_lr})")
        global_temp_tx = None  # 暂时不使用温度优化器
        
        logger.info("✅ 全局优化器创建成功")
        
        # **验证优化器一致性（调试pytree问题）** - 注释掉以加速初始化
        ...
        logger.info("✅ 跳过FSDP分片效果验证以加速初始化")
        
        # 创建轻量级agent用于trainer（避免重复创建模型）
        logger.info("🔧 创建轻量级Agent...")
        ...
        # Log GPU memory after FSDP creation and verify sharding - 注释掉以加速初始化
        ...
        logger.info("✅ 跳过GPU内存监控和FSDP分片验证以加速初始化")
        ...
        logger.info("✅ 跳过训练状态pytree一致性验证以加速初始化")
        
        logger.info("✅ 训练器创建成功，使用OpenPI标准8卡FSDP配置")
        
        # Log GPU memory before training starts - 注释掉以加速初始化
        # logger.info("🔍 GPU内存使用 - 训练开始前:")
        # log_gpu_memory("BEFORE_TRAINING")
        
        logger.info("✅ 跳过训练开始前GPU内存监控以加速初始化")
