...
# 根据CLAUDE.md原则：超时就增加时间！而不是简化测试！
# 增加XLA内存分配以充分利用GPU资源
export XLA_PYTHON_CLIENT_MEM_FRACTION=0.9  # 增加至0.9到0.95
export XLA_PYTHON_CLIENT_PREALLOCATE=true   # 启用预分配优化
...
echo "Command: nohup /era-ai/conda_envs/openpi/bin/uv run python scripts/train_acrlpd_pi0.py --config $CONFIG --exp_name $EXP_NAME --overwrite --no_wandb"
echo "Log file: $LOG_FILE"
echo ""

nohup /era-ai/conda_envs/openpi/bin/uv run python scripts/train_acrlpd_pi0.py \
    --config "$CONFIG" \
    --exp_name "$EXP_NAME" \
    --resume \
    --debug-transforms \
